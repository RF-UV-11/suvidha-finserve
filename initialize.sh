#!/bin/bash
# Initializes Suvidha FinServe: venv -> deps (incl. the weave SDK from a
# sibling `weave/` checkout, needed by onboard.py) -> compile the weave
...
#
# Config: DEMO_PORT (default 9102), WEAVE_REPO (default ../weave).
set -euo pipefail
...
echo "==> Starting Suvidha FinServe API on port ${DEMO_PORT:-9102}"
exec "$PYTHON" api.py
